// ============================================================
// Project				:	CityRPG
// Author				:	/Ty
// Description			:	Medical-Inturn Code file
// ============================================================

$CityRPG::jobs::name = "Medical Inturn";
$CityRPG::jobs::track = "Medical";
$CityRPG::jobs::initialInvestment = 40;
$CityRPG::jobs::pay = 50;
$CityRPG::jobs::tools = "PillItem PillItem";
$CityRPG::jobs::datablock = Player9SlotPlayer;
$CityRPG::jobs::education = 2;

$CityRPG::jobs::sellItems = false;
$CityRPG::jobs::sellFood = false;
$CityRPG::jobs::sellServices = false;

$CityRPG::jobs::law = true;
$CityRPG::jobs::canPardon = false;

$CityRPG::jobs::thief = false;
$CityRPG::jobs::hideJobName = false;

$CityRPG::jobs::offerer = false;
$CityRPG::jobs::claimer = false;

$CityRPG::jobs::labor = false;

$CityRPG::jobs::tmHexColor = "E50000";
$CityRPG::jobs::helpline = "\c6Medical Inturns' are the people that help the Doctors with medical equipment.";

$CityRPG::jobs::outfit = "none none none none blueShirt blueShirt skin bluePants blackShoes smileyFemale1 Hoodie";